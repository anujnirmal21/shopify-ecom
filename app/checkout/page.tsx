...
import { getCart, updateCartBuyerIdentity } from "@/lib/shopify";
...
export default async function CheckoutPage() {
  ...
  console.log(`[Checkout] Processing checkout for user: ${user.primaryEmailAddress?.emailAddress}`);
  ...
  if (!cartId) {
    console.warn(`[Checkout] No cart ID found in cookies. Redirecting to products.`);
    redirect("/products");
  }
  ...
  if (identityResult.userErrors.length > 0) {
    console.error(`[Checkout] Failed to update buyer identity:`, identityResult.userErrors);
